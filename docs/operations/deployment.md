...
本文档描述当前自动化发布链路：CI 检查、Gateway Docker 镜像、用户自部署 Gateway、Windows Release，以及可手动触发的 Ubuntu 22.04 Linux 构建。
...
| `v*` tag / 手动指定 tag | `.github/workflows/desktop-release.yml` | 基于指定 release tag 构建并发布 Windows x64 安装包。 |
| 手动触发 | `.github/workflows/linux-build.yml` | 在 Ubuntu 22.04 上构建 Linux x64 的 AppImage、`.deb` 和 `.rpm`，上传为 GitHub Actions artifact，不要求 release tag。 |
...
当前 Windows release 使用 Tauri updater 相关 secrets：
...
`desktop-release.yml` 当前只负责 Windows x64 release：

| 平台 | Runner | 产物 |
|---|---|---|
| Windows x64 | `windows-latest` | `LiveAgent-vX.Y.Z-Windows-x64.msi`、`LiveAgent-vX.Y.Z-Windows-x64-Setup.exe`，以及便携版 `.zip`。 |

Linux 构建由 `.github/workflows/linux-build.yml` 单独负责。该 workflow 只能手动触发，在 `ubuntu-22.04` 上构建 Linux x64 的 AppImage、`.deb` 和 `.rpm`，并将它们上传为 GitHub Actions artifact，不会自动创建 Release。
